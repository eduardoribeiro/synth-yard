...
const STATES = {
  '0': 'IDLE', free: 'IDLE', idle: 'IDLE', stopped: 'STOPPED',
  '1': 'PRINTING', print: 'PRINTING', printing: 'PRINTING',
  '2': 'FINISHED', complete: 'FINISHED', completed: 'FINISHED',
  '3': 'ERROR', failed: 'ERROR', error: 'ERROR',
  '4': 'STOPPED', abort: 'STOPPED', aborted: 'STOPPED', cancelled: 'STOPPED',
  '5': 'PAUSED', pause: 'PAUSED', paused: 'PAUSED',
};
...
function statusOf(raw) {
  ...
  if (device === 'PAUSED' || print === 'PAUSED' || raw.pause === 1 || raw.paused === 1) return 'PAUSED';
...
function normalize(raw) {
  ...
  const layer = number(raw.layer), total = number(raw.TotalLayer);
  ...
  const timeRemaining = progress > 2 && elapsed > 0
    ? Math.round(elapsed * (100 - progress) / progress) : null;
...
function exchange(printer, { command, confirm, timeout = 8000 } = {}) {
  return new Promise((resolve, reject) => {
    let ws, collectTimer, pollTimer, settled = false;
    const raw = {};
    const done = (err) => {
      if (settled) return;
      settled = true;
    ...
    const timer = setTimeout(() => done(new Error('Creality telemetry/command confirmation timed out')), timeout);
    try { ws = new WebSocket(address(printer).ws); } catch (err) { done(err); return; }
    const send = payload => {
      try { ws.send(JSON.stringify(payload)); } catch (err) { done(err); }
    };
    ...
    ws.addEventListener('message', ({ data }) => {
      ...
      for (const key of ['deviceState', 'state', 'pause', 'paused', 'printFileName', 'printProgress', 'dProgress', 'printJobTime', 'layer', 'TotalLayer']) {
    ...
    ws.addEventListener('close', () => { if (!settled) done(new Error('Creality WebSocket closed before confirmation')); });
  });
}

async function getStatus(printer) {
  try { return await exclusive(printer, async () => normalize(await exchange(printer))); }
...
async function uploadAndPrint(printer, gcodeFullPath, filename) {
  ...
  if (!filename || basename(filename) !== filename || /[\r\n\x00]/.test(filename) || !/\.gcode$/i.test(filename)) {
  ...
  return exclusive(printer, async () => {
    ...
    try {
      await exchange(printer, {
        ...
        confirm: raw => ['PRINTING', 'PAUSED'].includes(statusOf(raw)) && basename(raw.printFileName) === filename,
        timeout: 15000,
      });
    } catch (err) {
...
async function cancelJob(printer) {
  await exclusive(printer, () => exchange(printer, {
...
async function checkIfPrinting(printer, filename) {
  ...
  return ['PRINTING', 'PAUSED'].includes(result.status)
    && (!filename || result.currentFile === filename);
}
