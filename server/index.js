...
const server = app.listen(PORT, () => {
  ...
  app.post("/api/printers/set-ready-batch", (req, res) => {
    ...
    const printers = db
      .prepare(`SELECT * FROM printers WHERE id IN (${placeholders}) AND is_active = 1`)
      .all(...ids);
    const batchSetting = db
    ...
    console.log(
      `[server] Batch set-ready: ${printers.length} printer(s), target concurrency ${batchSize}`,
    );
    scheduler
      ._sweepInBatches(printers)
      .catch((err) => console.error("[scheduler] Batch set-ready sweep error:", err));
  ...
  app.post("/api/printers/:id/set-ready", (req, res) => {
    ...
    let finishedJob =
      uploadingJobEarly || printingJobEarly
        ? null
        : db
            .prepare(`
    ...
    if (finishedJob) {
      ...
      if (confirmed_qty != null) {
        ...
        if (!isNaN(confirmedQty) && confirmedQty !== finishedJob.parts_per_plate) {
          ...
          console.log(
            `[server] ${printer.name} confirmed ${confirmedQty}/${finishedJob.parts_per_plate} good (delta ${delta > 0 ? "+" : ""}${delta})`,
          );
        }
      }
    } else {
      ...
      if (activeJob) {
        ...
        if (printer.status === "OFFLINE" && activeJob.status === "printing") {
          console.log(
            `[server] ${printer.name} set ready from OFFLINE — job ${activeJob.id} still printing, no qty credited`,
          );
        } else {
          const creditQty =
        ...
        `).run(creditQty, now, activeJob.part_id);
          ...
          const label = printingJob
            ? "missed-finish"
            : activeJob.status === "cancelled"
              ? "cancelled-confirmed-good"
              : "MQTT-recovered finish";
          console.log(
            `[server] ${printer.name} ${label} confirmed good — Part "${part.name}" ${part.completed_qty}/${part.target_qty}`,
          );
          ...
          if (part.completed_qty >= part.target_qty) {
            ...
            console.log(
              `[server] Part "${part.name}" closed (${part.completed_qty}/${part.target_qty})`,
            );
        ...
        if (uploadingJob) {
          if (printer.status === "FINISHED" || printer.status === "IDLE") {
            ...
            db.prepare(
              "UPDATE jobs SET status = 'finished', finished_at = ?, started_at = COALESCE(started_at, ?) WHERE id = ?",
            ...
            console.log(
              `[server] ${printer.name} upload-stalled job ${uploadingJob.id} confirmed finished — Part "${part.name}" ${part.completed_qty}/${part.target_qty}`,
            );
            if (part.completed_qty >= part.target_qty) {
              db.prepare(`UPDATE parts SET status = 'closed', updated_at = ? WHERE id = ?`).run(
                now,
            ...
            db.prepare("UPDATE jobs SET status = 'printing', started_at = ? WHERE id = ?").run(
              now,
              uploadingJob.id,
            );
            console.log(
              `[server] ${printer.name} upload-stalled job ${uploadingJob.id} confirmed running by operator — changed to printing`,
            );
          }
        }
      }
