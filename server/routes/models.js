...
module.exports = (db) => {
  ...
  router.get("/", (_req, res) => {
    const models = db.prepare("SELECT * FROM printer_models ORDER BY connector, model_id").all();
    res.json(models);
  });
  ...
  router.post("/", (req, res) => {
    ...
    if (!VALID_CONNECTORS.includes(connector)) {
      return res
        .status(400)
        .json({ error: `connector must be one of: ${VALID_CONNECTORS.join(", ")}` });
    }
    // Normalize: lowercase, spaces → hyphens
    const id = model_id.trim().toLowerCase().replace(/\s+/g, "-");
    try {
      db.prepare("INSERT INTO printer_models (model_id, label, connector) VALUES (?, ?, ?)").run(
        id,
        label.trim(),
        connector,
      );
  ...
  router.delete("/:model_id", (req, res) => {
    ...
    const inUse = db
      .prepare("SELECT COUNT(*) as count FROM printers WHERE model = ? AND is_active = 1")
      .get(model_id);
    if (inUse.count > 0) {
