...
module.exports = (db, scheduler = null) => {
  router.get("/", (req, res) => {
    const projects = db
      .prepare("SELECT * FROM projects ORDER BY priority ASC, created_at ASC")
      .all();
    res.json(projects);
  });
  ...
  router.post("/:id/complete", (req, res) => {
    ...
    const now = Date.now();
    const openParts = db
      .prepare("SELECT * FROM parts WHERE project_id = ? AND status = 'open'")
      .all(project.id);

    db.prepare(
      "UPDATE parts SET status = 'closed', updated_at = ? WHERE project_id = ? AND status = 'open'",
    ).run(now, project.id);
    ...
    console.log(
      `[server] Project ${project.id} "${project.name}" force-completed — ${openParts.length} part(s) closed, ${cancelledJobs} job(s) cancelled`,
    );
  ...
  router.post("/:id/reactivate", (req, res) => {
    ...
    if (eligible.length > 0) {
      ...
      db.prepare(
        `UPDATE parts SET status = 'open', updated_at = ? WHERE id IN (${placeholders})`,
    ...
    console.log(
      `[server] Project ${project.id} "${project.name}" re-activated — ${eligible.length} part(s) reopened`,
    );
  ...
  router.post("/:id/duplicate", (req, res) => {
    ...
    db.transaction(() => {
      ...
      newProject = db
        ...
        .get(projResult.lastInsertRowid);
      ...
      for (const part of sourceParts) {
        ...
        `)
          .run(
            ...
            part.target_qty,
            part.sort_order,
            part.print_time_seconds ?? null,
            part.material_grams ?? null,
            now,
            now,
          );
        copiedParts++;
        ...
        for (const gcode of gcodes) {
          ...
          `).run(
            ...
            gcode.filename,
            newFilepath,
            gcode.parts_per_plate,
            gcode.est_print_secs ?? null,
            gcode.material_grams ?? null,
            gcode.ams_slot ?? null,
            now,
          );
          copiedGcodes++;
    ...
    res
      .status(201)
      .json({ project: newProject, copied_parts: copiedParts, copied_gcodes: copiedGcodes });
  });
