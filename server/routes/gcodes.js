...
function parseFilename(filename) {
  ...
  const regex =
    /^(\d+)x\s+(.+?)_(\d+\.\d+n)_(\d+\.\d+mm)_([A-Za-z]+)_([A-Za-z0-9]+)_(\d+h\d+m)(?:_[^.]+)?\.(bgcode|gcode)$/i;
...
module.exports = (db, scheduler = null) => {
  ...
  router.post("/upload", async (req, res) => {
    ...
    if (!part_id || !parts_per_plate || !printer_model) {
      ...
      return res
        .status(400)
        .json({ error: "part_id, parts_per_plate, and printer_model are required" });
    }

    if (!db.prepare("SELECT 1 FROM printer_models WHERE model_id = ?").get(printer_model)) {
      fs.unlinkSync(req.file.path);
      return res.status(400).json({
        error: `Unknown model "${printer_model}". Add it in Settings → Printer Models first.`,
      });
    }

    // Enforce uniqueness on (part_id, printer_model) at app layer
    const existing = db
      .prepare("SELECT id FROM gcodes WHERE part_id = ? AND printer_model = ?")
    ...
    const parsedRequiredMaterial =
      required_material && required_material !== "" ? required_material.trim() : null;
    const parsedRequiredColor =
      required_color && required_color !== "" ? required_color.trim() : null;
    ...
    res
      .status(201)
      .json(db.prepare("SELECT * FROM gcodes WHERE id = ?").get(gcode.lastInsertRowid));
  });
  ...
  router.put("/:id", (req, res) => {
    ...
    if ("print_time" in req.body) {
      if (!req.body.print_time) {
        ...
        if (estPrintSecs === null) {
          return res.status(400).json({
            error: 'Cannot parse print time. Use formats like "2h15m", "90m", or "1:30:00".',
          });
        }
      }
    }
    ...
    if ("material_grams" in req.body) {
      if (!req.body.material_grams) {
        ...
        if (materialGrams === null) {
          return res
            .status(400)
            .json({ error: 'Cannot parse material. Use formats like "45g", "45.5g", or "1.2kg".' });
        }
      }
    }
    ...
    const allowedGroups =
      "allowed_groups" in req.body ? req.body.allowed_groups || null : gcode.allowed_groups;
    const requiredMaterial =
      "required_material" in req.body
        ? req.body.required_material || null
        : gcode.required_material;
    const requiredColor =
      "required_color" in req.body ? req.body.required_color || null : gcode.required_color;

    db.prepare(
      "UPDATE gcodes SET est_print_secs = ?, material_grams = ?, allowed_groups = ?, required_material = ?, required_color = ? WHERE id = ?",
    ).run(
      estPrintSecs,
      materialGrams,
      allowedGroups,
  ...
  router.delete("/:id", (req, res) => {
    ...
    if (activeJob) {
      return res
        .status(409)
        .json({ error: "Cannot delete — this G-code has an active job in progress." });
    }
