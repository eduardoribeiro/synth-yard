...
module.exports = (db, scheduler = null) => {
  ...
  router.get("/", (req, res) => {
    ...
    const parts = project_id
      ? db
          .prepare(
            `SELECT parts.*, ${ACTIVE_QTY_SQL} FROM parts WHERE project_id = ? ORDER BY sort_order ASC, created_at ASC`,
          )
          .all(project_id)
      : db
          .prepare(
            `SELECT parts.*, ${ACTIVE_QTY_SQL} FROM parts ORDER BY sort_order ASC, created_at ASC`,
          )
          .all();
    res.json(parts);
  });
  ...
  router.get("/:id", (req, res) => {
    const part = db
      .prepare(`SELECT parts.*, ${ACTIVE_QTY_SQL} FROM parts WHERE parts.id = ?`)
      .get(req.params.id);
  ...
  router.get("/:id/dispatch-status", (req, res) => {
    ...
    if (part.status === "open" && part.active_qty >= remaining && remaining > 0) {
      blockers.push(
        `Jobs already printing cover the remaining ${remaining} part(s) — waiting for them to finish`,
      );
    }
    ...
    for (const gc of gcodes) {
      ...
      const allowedGroups = gc.allowed_groups
        ...
        : part.project_allowed_groups
          ? JSON.parse(part.project_allowed_groups)
          : null;

      const modelPrinters = db
        .prepare("SELECT * FROM printers WHERE model = ? AND is_active = 1")
      ...
      const groupOk = modelPrinters.filter(
        (p) => !allowedGroups || allowedGroups.includes(p.group_name),
      );
      if (groupOk.length === 0) {
      ...
      const materialOk = groupOk.filter(
        (p) =>
      ...
      if (materialOk.length === 0) {
        ...
        notes.push(
          `${gc.filename}: no printer has ${want} loaded (set it on the printer's detail page)`,
        );
        continue;
      }
      ...
      const ready = materialOk.filter(
        (p) =>
          (p.status === "IDLE" || p.status === "FINISHED" || p.status === "STOPPED") &&
          p.is_held === 0,
      );
      if (ready.length === 0) {
  ...
  router.post("/", (req, res) => {
    ...
    const maxRow = db
      .prepare("SELECT MAX(sort_order) AS max FROM parts WHERE project_id = ?")
      .get(project_id);
    ...
    res
      .status(201)
      .json(db.prepare("SELECT * FROM parts WHERE id = ?").get(result.lastInsertRowid));
  });
  ...
  router.put("/:id", (req, res) => {
    ...
    if (part.status === "closed" && resolvedStatus === "open") {
      ...
      if (project && project.status === "completed") {
        ...
        console.log(
          `[parts] Project ${project.id} reopened — part ${part.id} target_qty raised above completed_qty`,
        );
        if (scheduler) scheduler.sweepIdlePrinters();
      }
    }
  ...
  router.delete("/:id", (req, res) => {
    ...
    if (activeJob) {
      return res
        .status(409)
        .json({ error: "Cannot delete — this part has an active job in progress." });
    }
