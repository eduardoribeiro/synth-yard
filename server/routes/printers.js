...
module.exports = (db) => {
  ...
  router.get("/decommissioned", (req, res) => {
    const printers = db
      .prepare("SELECT * FROM printers WHERE is_active = 0 ORDER BY decommissioned_at DESC")
      .all();
    res.json(printers);
  });
  ...
  router.post("/", (req, res) => {
    ...
    if (
      !normalized ||
      !db.prepare("SELECT 1 FROM printer_models WHERE model_id = ?").get(normalized)
    ) {
      return res
        .status(400)
        .json({ error: `Unknown model "${model}". Add it in Settings → Printer Models first.` });
    }
    ...
    try {
      ...
      if (group_name && group_name.trim()) {
        try {
          registerGroup.run(group_name.trim(), Date.now());
        } catch (_) {}
      }
      res
        .status(201)
        .json(db.prepare("SELECT * FROM printers WHERE id = ?").get(result.lastInsertRowid));
    } catch (err) {
  ...
  router.put("/:id", (req, res) => {
    ...
    const newMaterial =
      "loaded_material" in req.body ? loaded_material || null : printer.loaded_material;
  ...
  router.post("/:id/decommission", (req, res) => {
    ...
    if (!printer) return res.status(404).json({ error: "Printer not found" });
    const now = Date.now();
    db.prepare("UPDATE printers SET is_active = 0, decommissioned_at = ? WHERE id = ?").run(
      now,
      printer.id,
    );
  ...
  router.post("/:id/complete-and-decommission", (req, res) => {
    ...
    const parsedQty =
      confirmed_qty != null && !isNaN(parseInt(confirmed_qty, 10))
    ...
    const settlePart = (partId) => {
      ...
      if (part.completed_qty >= part.target_qty && part.status === "open") {
        db.prepare(`UPDATE parts SET status = 'closed', updated_at = ? WHERE id = ?`).run(
          now,
          part.id,
        );
        db.prepare(
        ...
        const openCount = db
          .prepare(`SELECT COUNT(*) AS count FROM parts WHERE project_id = ? AND status = 'open'`)
          .get(part.project_id).count;
        if (openCount === 0) {
          db.prepare(`UPDATE projects SET status = 'completed', updated_at = ? WHERE id = ?`).run(
            now,
            part.project_id,
          );
        ...
        db.prepare(`UPDATE parts SET status = 'open', updated_at = ? WHERE id = ?`).run(
          now,
          part.id,
        );
        ...
        if (project && project.status === "completed") {
          ...
          console.log(
            `[printers] Project ${project.id} reopened — confirmed qty reduced on decommission`,
          );
        }
      }
    };
    ...
    if (printingJob) {
      ...
      db.prepare(`UPDATE jobs SET status = 'finished', finished_at = ? WHERE id = ?`).run(
        now,
        printingJob.id,
      );
      db.prepare(
      ...
      console.log(
        `[printers] ${printer.name} missed-finish credited ${creditQty} — decommissioning for maintenance`,
      );
      ...
      if (finishedJob && parsedQty !== finishedJob.parts_per_plate) {
        ...
        console.log(
          `[printers] ${printer.name} confirmed ${parsedQty}/${finishedJob.parts_per_plate} good on decommission (delta ${delta > 0 ? "+" : ""}${delta})`,
        );
      }
    }
    ...
    db.prepare(
      "UPDATE printers SET is_active = 0, is_held = 0, decommissioned_at = ?, decommission_note = ? WHERE id = ?",
    ).run(now, decommNote, printer.id);
    events.insert(
      printer.id,
      "decommission",
      decommNote ?? "operator confirmed successful print — taken offline for maintenance",
    );
  ...
  router.post("/:id/mark-job-failure", (req, res) => {
    ...
    if (!job) {
      ...
      db.prepare(
        "UPDATE printers SET is_active = 0, decommissioned_at = ?, decommission_note = ? WHERE id = ?",
      ).run(now, noJobNote, printer.id);
      events.insert(
        printer.id,
        "job_failed",
        noJobNote ?? "No tracked job — printer decommissioned for investigation",
      );
    ...
    console.log(
      `[printers] Job ${job.id} marked failed — ${printer.name} decommissioned pending investigation`,
    );
  ...
  router.get("/:id/raw-status", async (req, res) => {
    ...
    try {
      ...
      res.json({
        printer: { id: printer.id, name: printer.name, ip: printer.ip },
        error: err.message,
      });
    }
  });
  ...
  router.post("/import", upload.single("file"), (req, res) => {
    ...
    for (const row of rows) {
      ...
      try {
        ...
        if (group_name) {
          try {
            registerGroup.run(group_name, now);
          } catch (_) {}
        }
        summary.imported++;
      } catch (err) {
  ...
  router.post("/:id/link-job", (req, res) => {
    ...
    if (!["failed", "uploading"].includes(job.status)) {
      return res
        .status(409)
        .json({ error: `Job is in '${job.status}' status and cannot be linked` });
    }
