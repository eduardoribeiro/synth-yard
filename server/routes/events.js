...
module.exports = (db) => {
  ...
  router.get("/", (req, res) => {
    ...
    const events = db
      .prepare("SELECT * FROM printer_events WHERE printer_id = ? ORDER BY created_at DESC")
      .all(req.params.id);
    res.json(events);
  });
  ...
  router.post("/", (req, res) => {
    ...
    res
      .status(201)
      .json(db.prepare("SELECT * FROM printer_events WHERE id = ?").get(result.lastInsertRowid));
  });
