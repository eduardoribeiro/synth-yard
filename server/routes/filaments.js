...
module.exports = (db) => {
  ...
  router.post("/types", (req, res) => {
    ...
    try {
      ...
      res
        .status(201)
        .json(db.prepare("SELECT * FROM filament_types WHERE id = ?").get(result.lastInsertRowid));
    } catch (err) {
  ...
  router.delete("/types/:id", (req, res) => {
    const colorCount = db
      .prepare("SELECT COUNT(*) as count FROM filament_colors WHERE type_id = ?")
      .get(req.params.id);
    if (colorCount.count > 0) {
      return res.status(409).json({
        error: `Cannot delete — ${colorCount.count} color(s) belong to this type. Delete them first.`,
      });
    }
  ...
  router.post("/colors", (req, res) => {
    ...
    try {
      const result = db
        .prepare("INSERT INTO filament_colors (type_id, name, hex_color) VALUES (?, ?, ?)")
        .run(type_id, name, hex);
      res.status(201).json(
        db
          .prepare(`
        SELECT fc.*, ft.name AS type_name
      ...
      if (err.message.includes("UNIQUE"))
        return res.status(409).json({ error: `"${name}" already exists for this type` });
      throw err;
    }
  });
