...
function seedPrinter(name = `Printer_${Date.now()}`) {
  const now = Date.now();
  return db
    .prepare(`INSERT INTO printers (name, ip, model, created_at) VALUES (?, '10.0.0.1', 'mk4s', ?)`)
...
describe("GET /api/printers/:id/events", () => {
  ...
  test("returns events newest first", async () => {
    ...
    db.prepare(
      `INSERT INTO printer_events (printer_id, event_type, note, created_at) VALUES (?, 'note', 'first', ?)`,
    ).run(id, 1000);
    db.prepare(
      `INSERT INTO printer_events (printer_id, event_type, note, created_at) VALUES (?, 'note', 'second', ?)`,
    ).run(id, 2000);
    db.prepare(
      `INSERT INTO printer_events (printer_id, event_type, note, created_at) VALUES (?, 'note', 'third', ?)`,
    ).run(id, 3000);
...
describe("POST /api/printers/:id/events", () => {
  test("returns 404 for unknown printer", async () => {
    const res = await request(app).post("/api/printers/99999/events").send({ note: "hello" });
    expect(res.status).toBe(404);
  });
  ...
  test("returns 400 when note is blank", async () => {
    ...
    const res = await request(app).post(`/api/printers/${id}/events`).send({ note: "   " });
    expect(res.status).toBe(400);
  });
  ...
  test("created event appears in subsequent GET", async () => {
    ...
    await request(app).post(`/api/printers/${id}/events`).send({ note: "Hotend cleaned" });
