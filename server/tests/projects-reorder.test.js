...
describe("PUT /api/projects/reorder", () => {
  ...
  beforeAll(() => {
    ...
    idA = db
      .prepare(
        "INSERT INTO projects (name, priority, created_at, updated_at) VALUES ('Alpha', 0, ?, ?)",
      )
      .run(now, now).lastInsertRowid;
    idB = db
      .prepare(
        "INSERT INTO projects (name, priority, created_at, updated_at) VALUES ('Beta',  1, ?, ?)",
      )
      .run(now, now).lastInsertRowid;
    idC = db
      .prepare(
        "INSERT INTO projects (name, priority, created_at, updated_at) VALUES ('Gamma', 2, ?, ?)",
      )
      .run(now, now).lastInsertRowid;
  });
