...
function findPayload(command) {
  const call = mockPublish.mock.calls.find((c) => {
    try {
      return JSON.parse(c[1]).print?.command === command;
    } catch {
      return false;
    }
  });
...
describe("getAmsSlots", () => {
  ...
  test("always includes external spool as slot -1", () => {
    ...
    expect(bambu.getAmsSlots(printer)).toContainEqual({ slot: -1, type: "ABS", color: "000000FF" });
  });
...
describe("uploadAndPrint — .3mf (project_file)", () => {
  ...
  test("throws when MQTT is not connected", async () => {
    ...
    await expect(bambu.uploadAndPrint(printer, "/tmp/1234_part.3mf", "part.3mf")).rejects.toThrow(
      /MQTT not connected/,
    );
  });
