...
describe("withQuery", () => {
  test("preserves empty values required by Moonraker object queries", () => {
    expect(
      withQuery("http://printer.local/printer/objects/query", {
        print_stats: "",
        virtual_sdcard: "",
        webhooks: "",
      }),
  ...
  test("omits null and undefined values", () => {
    expect(
      withQuery("http://printer.local/status", { present: "yes", no: null, missing: undefined }),
...
describe("requestEmpty and requestRaw", () => {
  ...
  test("enables duplex mode for a Node readable upload stream", async () => {
    ...
    await requestEmpty("http://printer.local/upload", {
      method: "PUT",
      body: Readable.from("gcode"),
    });
    expect(global.fetch).toHaveBeenCalledWith(
      "http://printer.local/upload",
