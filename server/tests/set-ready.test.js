...
function seedJob(
  ...
  gcodeId,
  jobStatus = "finished",
  { partsPerPlate = 4, startedAt, finishedAt } = {},
) {
  const now = Date.now();
...
describe("set-ready — normal finish (job already finished)", () => {
  ...
  test("calls scheduleForPrinter after releasing hold", async () => {
    ...
    await request(makeApp(db, scheduler)).post(`/api/printers/${printerId}/set-ready`).send({});
...
describe("set-ready — missed finish (printing job)", () => {
  test("credits completed_qty for the printing job", async () => {
    ...
    await request(makeApp(db)).post(`/api/printers/${printerId}/set-ready`).send({});
