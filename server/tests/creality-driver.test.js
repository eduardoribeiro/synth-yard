...
describe("Creality status", () => {
  ...
  test("keeps a preparing printer busy despite zero targets and stale stopped print state", async () => {
    scenarios.push({
      frames: [{ deviceState: 1, state: 0, targetNozzleTemp: 0, targetBedTemp: 0 }],
    });
  ...
  test.each([{ fail: "error" }, { fail: "close" }, { frames: ["ok", {}, { printProgress: 10 }] }])(
    "reports OFFLINE for failed or incomplete telemetry: %j",
    async (scenario) => {
...
describe("Creality upload and control", () => {
    ...
    scenarios.push(
      { frames: [{ deviceState: 0 }] },
      {
        frames: [
          "ok",
          { deviceState: 1 },
          { printFileName: "/usr/data/printer_data/gcodes/my part.gcode" },
        ],
      },
    );
    await settle(
      driver.uploadAndPrint({ ...printer, ip: "http://192.168.1.50:8080/" }, file, "my part.gcode"),
    );
  ...
  test.each(["part.bgcode", "part.3mf", "../part.gcode", "dir\\part.gcode", "bad\n.gcode"])(
    "rejects invalid filename %s before contacting the printer",
    async (filename) => {
      await expect(driver.uploadAndPrint(printer, file, filename)).rejects.toMatchObject({
        retryable: false,
        recoverable: false,
      });
  ...
  test.each([1, 5, 3, "unrecognized"])(
    "does not upload to a printer with state %s",
    async (deviceState) => {
      scenarios.push({ frames: [{ deviceState }] });
      await expect(
        settle(driver.uploadAndPrint(printer, file, "part.gcode")),
      ).rejects.toMatchObject({ retryable: false, recoverable: false });
      expect(httpMock.post).not.toHaveBeenCalled();
    },
  );
  ...
  test.each(
    [
      ["ok"],
      [{ deviceState: 1, printFileName: "other.gcode" }],
      [{ deviceState: 0, printFileName: "part.gcode" }],
    ].map((frames) => [frames]),
  )("an unconfirmed start cannot be retried (%j)", async (frames) => {
    scenarios.push({ frames: [{ deviceState: 0 }] }, { frames });
    await expect(settle(driver.uploadAndPrint(printer, file, "part.gcode"))).rejects.toMatchObject({
      retryable: false,
    });
    expect(sockets.every((s) => s.closed)).toBe(true);
  });
