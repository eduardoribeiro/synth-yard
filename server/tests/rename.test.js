...
beforeAll(() => {
  ...
  db.prepare(
    "INSERT INTO parts (project_id, name, target_qty, created_at, updated_at) VALUES (1, ?, 50, ?, ?)",
  ).run("Original Part", now, now);
...
describe("PUT /api/projects/:id — rename", () => {
  test("renames a project", async () => {
    const res = await request(app).put("/api/projects/1").send({ name: "Renamed Project" });
  ...
  test("omitting name leaves existing name intact (COALESCE)", async () => {
    ...
    const res = await request(app).put("/api/projects/1").send({ status: "active" });
  ...
  test("returns 404 for unknown project id", async () => {
    const res = await request(app).put("/api/projects/9999").send({ name: "Ghost" });
...
describe("PUT /api/parts/:id — rename", () => {
  test("renames a part", async () => {
    const res = await request(app).put("/api/parts/1").send({ name: "Renamed Part" });
  ...
  test("omitting name leaves existing name intact (COALESCE)", async () => {
    ...
    const res = await request(app).put("/api/parts/1").send({ completed_qty: 5 });
  ...
  test("returns 404 for unknown part id", async () => {
    const res = await request(app).put("/api/parts/9999").send({ name: "Ghost Part" });
