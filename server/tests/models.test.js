...
describe("POST /api/models", () => {
  ...
  test("rejects missing model_id", async () => {
    const res = await request(app).post("/api/models").send({ label: "MK4", connector: "prusa" });
    expect(res.status).toBe(400);
  ...
  test("rejects missing connector", async () => {
    const res = await request(app).post("/api/models").send({ model_id: "mk4", label: "MK4" });
    expect(res.status).toBe(400);
    expect(res.body.error).toMatch(/required/i);
  });
...
describe("DELETE /api/models/:model_id", () => {
  test("deletes a model and returns ok", async () => {
    ...
    expect(
      db.prepare("SELECT * FROM printer_models WHERE model_id = ?").get("mk4"),
    ).toBeUndefined();
  });
