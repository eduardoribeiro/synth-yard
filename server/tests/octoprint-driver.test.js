...
describe("uploadAndPrint", () => {
  ...
  test("throws UPLOAD_CONFLICT on 409 response", async () => {
    ...
    await expect(octoprint.uploadAndPrint(fakePrinter, fullPath, filename)).rejects.toMatchObject({
  ...
  test("rethrows non-409 errors unchanged", async () => {
    ...
    await expect(octoprint.uploadAndPrint(fakePrinter, fullPath, filename)).rejects.toThrow("500");
  });
});
