...
class JobScheduler extends EventEmitter {
  ...
  sweepIdlePrinters() {
    ...
    const eligiblePrinters = this.db
      .prepare(`
    ...
    console.log(
      `[scheduler] Sweeping ${eligiblePrinters.length} eligible printer(s) (IDLE, operator-confirmed FINISHED, or resolved STOPPED)`,
    );
  ...
  async _sweepInBatches(printers) {
    ...
    if (this._isSweeping) {
      ...
      console.log(
        `[scheduler] Sweep in progress — ${printers.length} printer(s) deferred to end of current sweep`,
      );
      return;
    }
    ...
    try {
      ...
      while (toDispatch.length > 0) {
        const setting = this.db
          .prepare("SELECT value FROM settings WHERE key = 'dispatch_batch_size'")
          .get();
        ...
        if (activeJobIds.length > 0) {
          console.log(
            `[scheduler] Wave: ${activeJobIds.length}/${batchSize} printer(s) actually dispatching (considered ${consideredCount})`,
          );
          await this._waitForBatch(activeJobIds);
        } else if (consideredCount > 0) {
          console.log(
            `[scheduler] Wave: none of ${consideredCount} considered printer(s) had a dispatchable candidate`,
          );
        }
  ...
  scheduleForPrinter(printer) {
    if (this._isSweeping) {
      ...
      console.log(
        `[scheduler] ${printer.name} became dispatchable during a sweep, deferred to end of sweep`,
      );
      return;
    }
  ...
  _reserveJob(printer) {
    ...
    const fresh = this.db
      .prepare("SELECT is_held, status FROM printers WHERE id = ?")
      .get(printer.id);
    ...
    if (activeJob) {
      ...
      if (isStaleEligible && jobAge > STALE_JOB_GRACE_MS) {
        ...
        console.warn(
          `[scheduler] ${printer.name} stale job ${activeJob.id} auto-failed — printer is ${fresh.status}, held for operator review`,
        );
      } else if (isStaleEligible) {
        console.log(
          `[scheduler] ${printer.name} has a freshly dispatched job ${activeJob.id} (${Math.round(jobAge / 1000)}s old, printer ${fresh.status}) — skipping duplicate dispatch, not yet stale`,
        );
      } else {
        console.log(
          `[scheduler] ${printer.name} already has an active job — skipping duplicate dispatch`,
        );
      }
      return null;
    }
    ...
    try {
      ...
      this.db.prepare("UPDATE printers SET is_held = 1 WHERE id = ?").run(printer.id);
      console.error(
        `[scheduler] ${printer.name} has unknown type "${printer.type}" — held, no job created: ${err.message}`,
      );
      return null;
    }
    ...
    while (true) {
      ...
      `)
        .get(
          ...
          printer.loaded_color,
          ...skippedPartIds,
        );

      if (!candidate) {
        console.log(
          `[scheduler] No candidate found for ${printer.name} (model: ${printer.model}) — no open parts with matching G-code in an active project`,
        );
        return null;
      }
      ...
      if (inProgressParts - candidate.parts_per_plate >= remainingParts) {
        ...
        console.log(
          `[scheduler] Ceiling hit for part ${candidate.part_id} (${inProgressParts - candidate.parts_per_plate} of ${remainingParts} parts already in progress) — trying next part for ${printer.name}`,
        );
      ...
      if (!fs.existsSync(gcodeFullPath)) {
        ...
        const part = this.db
          .prepare(
            "SELECT parts.name, projects.name AS project_name FROM parts JOIN projects ON projects.id = parts.project_id WHERE parts.id = ?",
          )
          .get(candidate.part_id);
        notifications.add(
          `G-code file missing for "${candidate.filename}" — re-upload the file for part "${part?.name}" in project "${part?.project_name}".`,
        );
        console.warn(
          `[scheduler] G-code missing for part ${candidate.part_id} ("${candidate.filename}") — skipping to next part for ${printer.name}`,
        );
        skippedPartIds.push(candidate.part_id);
        continue;
      }
  ...
  async _executeUpload(printer, reservation) {
    ...
    if (lastErr) {
      ...
      const isActuallyPrinting =
        lastErr.recoverable !== false &&
        (await driver.checkIfPrinting(printer, candidate.filename));
      if (isActuallyPrinting) {
        this.db
          .prepare(`UPDATE jobs SET status = 'printing', started_at = ? WHERE id = ?`)
          .run(Date.now(), jobId);
        console.log(
          `[scheduler] ${printer.name} upload appeared to fail but printer is printing — job ${jobId} recovered`,
        );
        return jobId;
      }
      ...
      console.error(
        `[scheduler] ${printer.name} upload failed after ${attempts} attempt(s) — held, job ${jobId} left as uploading for operator confirmation`,
      );
      return null;
    }
  ...
  _handleFinished(printer) {
    ...
    if (!job) {
      ...
      if (job) {
        console.log(
          `[scheduler] FINISHED on ${printer.name} — recovering job ${job.id} (marked failed during this session, likely transient MQTT disconnect during print)`,
        );
      }
    }
    ...
    if (!job) {
      console.warn(
        `[scheduler] FINISHED on ${printer.name} but no printing job found — may be outside system`,
      );
    ...
    // Mark job finished
    this.db
      .prepare(`UPDATE jobs SET status = 'finished', finished_at = ? WHERE id = ?`)
      .run(now, job.id);
    ...
    console.log(
      `[scheduler] ${printer.name} finished — Part "${part.name}" ${part.completed_qty}/${part.target_qty}`,
    );
    ...
    events.insert(
      printer.id,
      "job_finished",
      `Job ${job.id} — ${part.name} (${job.parts_per_plate} parts)`,
    );
  ...
  _closePart(part, now) {
    this.db
      .prepare(`UPDATE parts SET status = 'closed', updated_at = ? WHERE id = ?`)
      .run(now, part.id);
    ...
    console.log(
      `[scheduler] Part "${part.name}" closed (${part.completed_qty}/${part.target_qty})`,
    );
    ...
    if (openCount === 0) {
      this.db
        .prepare(`UPDATE projects SET status = 'completed', updated_at = ? WHERE id = ?`)
  ...
  _handlePrinterOffline(printer) {
    ...
    if (activeJob) {
      ...
      events.insert(
        printer.id,
        "offline_with_job",
        `Printer went offline with job ${activeJob.id} in progress — awaiting operator confirmation`,
      );
      console.warn(
        `[scheduler] ${printer.name} went OFFLINE with active job — held for operator review (job left as printing)`,
      );
    } else {
  ...
  _handleRecoveredToPrinting(printer) {
    ...
    const activeJob = this.db
      .prepare("SELECT id FROM jobs WHERE printer_id = ? AND status = 'printing' LIMIT 1")
      .get(printer.id);
    ...
    if (activeJob) {
      ...
      events.insert(
        printer.id,
        "recovered",
        `Printer came back online and resumed printing — hold released automatically`,
      );
      console.log(
        `[scheduler] ${printer.name} auto-unhold — came back online and is printing (job ${activeJob.id})`,
      );
    }
  }
  ...
  _handlePrinterStopped(printer) {
    ...
    if (job) {
      this.db
        .prepare("UPDATE jobs SET status = 'cancelled', finished_at = ? WHERE id = ?")
        .run(Date.now(), job.id);
      events.insert(
        printer.id,
        "job_cancelled",
        `Job ${job.id} — stopped by operator on printer screen`,
      );
  ...
  _handlePrinterUnavailable(printer) {
    ...
    if (job) {
      this.db
        ...
        .run(Date.now(), job.id);
      console.warn(
        `[scheduler] Marked job ${job.id} failed — ${printer.name} went ${printer.status}`,
      );
    }
    ...
    this.db.prepare("UPDATE printers SET is_held = 1 WHERE id = ?").run(printer.id);
    console.warn(
      `[scheduler] ${printer.name} held — entered ${printer.status}, operator confirmation required`,
    );
  }
}
