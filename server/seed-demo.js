...
const benchyPartId = Number(
  insertPart.run(benchyProjId, "Standard Benchy", 100, 47, "open", 0, now - 14 * day, now - hr)
    .lastInsertRowid,
);
const miniBenchyPartId = Number(
  insertPart.run(benchyProjId, "Mini Benchy (60%)", 50, 12, "open", 1, now - 14 * day, now - 2 * hr)
    .lastInsertRowid,
);
const grid2x4PartId = Number(
  insertPart.run(
    gridfinityProjId,
    "2x4 Gridfinity Bin",
    200,
...
const clipPartId = Number(
  insertPart.run(cableProjId, "Clip Body", 500, 0, "open", 0, now - 3 * day, now - 3 * day)
    .lastInsertRowid,
);
...
insertJob.run(
  benchyPartId,
  printerIds["MK4S_04"],
  gcodeIds["benchy_4up_mk4s.bgcode"],
  4,
  "printing",
  now - 3 * hr,
  null,
...
insertEvent.run(
  printerIds["MK4S_01"],
  "job_done",
  "4 parts credited — print confirmed OK",
  now - 12 * day + 3 * hr,
);
insertEvent.run(
  printerIds["MK4S_01"],
...
insertEvent.run(
  printerIds["MK4S_04"],
  "job_done",
  "2 parts credited — print confirmed OK",
  now - 4 * day + 4 * hr,
);
...
insertEvent.run(
  printerIds["X1C_01"],
  "job_done",
  "8 parts credited — AMS slot 1 (grey PETG)",
  now - 4 * day + 1.5 * hr,
);
insertEvent.run(
  printerIds["X1C_01"],
  "job_start",
  "gridfinity_2x4_x1c.3mf — AMS slot 1",
  now - 1.5 * hr,
);
