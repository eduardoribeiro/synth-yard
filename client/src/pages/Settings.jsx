...
const CREDENTIAL_HELP = {
  creality:
    "No API key or root needed. Enter the printer IP; stock LAN uses HTTP port 80 and WebSocket port 9999. Ender 3 V3 KE protocol; K1 series experimental. K2 and CR-M4 are not supported. Upload plain .gcode files sliced for this model.",
  prusa:
    "API Key: on the printer under Settings → Network → PrusaLink, or in the PrusaLink web UI (open the printer's IP in a browser) under Settings → API Key.",
  "elegoo-centauri":
    "No API key needed — just the printer's IP address, shown on the printer's network settings screen.",
  "elegoo-centauri2":
    "Enable LAN mode on the printer. The Access Code and Serial Number are shown on the printer's network settings screen.",
  bambu:
    "Enable LAN Mode on the printer first. The Access Code is on the printer screen under Settings → WLAN; the Serial Number is under Settings → Device.",
  klipper:
    "No API key needed — just the IP of the machine running Moonraker. Port 7125 is used automatically.",
  octoprint:
    "API Key: in OctoPrint under Settings → API. If OctoPrint isn't on port 80 (commonly :5000), include the port in the IP field, e.g. 192.168.1.50:5000.",
};
      ...
      fetch("/api/settings")
        ...
        .then((settingsData) => {
          ...
          window.dispatchEvent(
            new CustomEvent("farmNameChanged", { detail: settingsData.farm_name || "Synth Yard" }),
          );
        })
        .catch(() => {});
    } catch (err) {
    ...
    <div>
      ...
      >
        ...
        <p style={{ color: "#64748b", fontSize: 13, marginBottom: 16 }}>
          Configure which printer models are available in your farm. Models appear in the G-code
          upload selector and the Add Printer form. Deleting a model is blocked if active printers
          use it.
        </p>
              ...
              {allModels.map((m) => (
                <tr key={m.model_id} style={{ borderBottom: "1px solid #1a2030" }}>
                  ...
                  <td style={{ padding: "6px 8px", color: "#64748b" }}>
                    {CONNECTOR_LABEL[m.connector] || m.connector}
                  </td>
        ...
        {allModels.length === 0 && (
          <p style={{ color: "#475569", fontSize: 13, marginBottom: 16 }}>
            No models configured yet. Add your first model below.
          </p>
        )}
      ...
      >
        ...
        <p style={{ color: "#64748b", fontSize: 13, marginBottom: 16 }}>
          Named groups (e.g. racks or rooms) that projects and G-codes can restrict dispatch to. A
          group stays listed here even when no printer currently carries it (assigning a printer's
          Group field on the Printers page auto-registers a new name here too). Deleting a group is
          blocked while any active printer, G-code, or project still references it.
        </p>
        ...
        {allGroups.length === 0 && (
          <p style={{ color: "#475569", fontSize: 13, marginBottom: 16 }}>
            No groups registered yet. Add one below, or it'll be created automatically the first
            time you type it on a printer.
          </p>
        )}
      ...
      >
        ...
        <p style={{ color: "#64748b", fontSize: 13, marginBottom: 20 }}>
          Define the filament types and colors available in your farm. Printers and G-codes select
          from these lists.
        </p>
        ...
        {typeFormError && (
          <div style={{ marginTop: -16, marginBottom: 16, color: "#fca5a5", fontSize: 13 }}>
            {typeFormError}
          </div>
        )}
        ...
        {filamentColors.length === 0 && (
          <p style={{ color: "#475569", fontSize: 13, marginBottom: 12 }}>
            No colors yet. Add your first below.
          </p>
        )}
        <form
        ...
        {colorFormError && (
          <div style={{ marginTop: 8, color: "#fca5a5", fontSize: 13 }}>{colorFormError}</div>
        )}
      </section>
          ...
          >
            ...
            <div>
              ...
              >
                {allModels.filter((m) => m.connector === addForm.type).length === 0 ? (
                  ...
                  allModels
                    .filter((m) => m.connector === addForm.type)
                    .map((m) => (
              ...
              <input
                ...
                placeholder={
                  addForm.type === "elegoo-centauri"
                    ? "Centauri_01"
                    : addForm.type === "elegoo-centauri2"
                      ? "CC2_01"
                      : addForm.type === "bambu"
                        ? "Bambu_X1C_01"
                        : addForm.type === "klipper"
                          ? "Voron_01"
                          : addForm.type === "octoprint"
                            ? "OctoPi_01"
                            : addForm.type === "creality"
                              ? "K1_01"
                              : "MK4S_11"
                }
                style={inputStyle}
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setAddForm((p) => ({ ...p, loaded_material: e.target.value, loaded_color: "" }))
                }
                style={inputStyle}
              >
      ...
      >
        ...
        <p style={{ color: "#64748b", fontSize: 13, marginBottom: 16 }}>
          ...
          <code style={{ color: "#94a3b8" }}>model, name, ip, api_key, group, type</code>.<br />
          Model field value is the ID from the Printer Models list above. Missing mandatory fields
          and duplicate names are skipped.
        </p>
                    ...
                    {f.reason.includes("Cannot infer model") && (
                      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                        <select
                          ...
                          onChange={(e) =>
                            setFlaggedModels((prev) => ({ ...prev, [i]: e.target.value }))
                          }
                          style={{
                            background: "#0f172a",
                            border: "1px solid #334155",
                        ...
                        <button
                          onClick={() =>
                            handleSaveFlagged(f, flaggedModels[i] || allModels[0]?.model_id || "")
                          }
                          style={{
                            background: "#15803d",
                            color: "#fff",
      ...
      >
        ...
        <p style={{ color: "#64748b", fontSize: 13, marginBottom: 16 }}>
          The scheduler keeps this many printers uploading or printing at once, pulling further down
          the ready queue to fill that target even if some printers have no matching work right now.
          Reduce this number if your network is saturated during large uploads.
        </p>
        ...
        {restoreResult && (
          <div style={{ marginTop: 14 }}>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              ...
              <Chip
                color="#4ade80"
                label={`${restoreResult.filament_colors ?? 0} filament colors`}
              />
            </div>
          </div>
        )}
      ...
      <section style={{ background: "#1e2433", borderRadius: 10, padding: 20, maxWidth: 640 }}>
        ...
        <p style={{ color: "#64748b", fontSize: 13 }}>
          All printers are polled every <strong style={{ color: "#e2e8f0" }}>15 seconds</strong> via
          their connector API. Polling runs concurrently — all printers are queried in parallel each
          tick. Unreachable printers show as <span style={{ color: "#6b7280" }}>OFFLINE</span> and
          do not affect other printers.
        </p>
      </section>
