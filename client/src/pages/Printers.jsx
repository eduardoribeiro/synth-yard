...
function summarize(group) {
  ...
  for (const p of group) {
    ...
    const awaiting =
      p.is_held === 1 && (p.status === "FINISHED" || p.status === "IDLE" || p.status === "STOPPED");
    if (awaiting) {
      counts.AWAITING++;
      continue;
    }
...
export default function Printers() {
  ...
  const [showDecom, setShowDecom] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem(SHOW_DECOM_KEY) || "false");
    } catch {
      return false;
    }
  });
  ...
  useEffect(() => {
    ...
    fetch("/api/groups")
      .then((r) => r.json())
      .then((groups) => setRegistryGroups(groups.map((g) => g.name)))
      .catch(() => {});
  }, [fetchPrinters]);
  ...
  const { groups, totalShown, totalMatched, decomGroup } = useMemo(() => {
    ...
    const decomMatched = decomPrinters.filter(matches).sort((a, b) => a.name.localeCompare(b.name));
    const decomGroup = {
  ...
  const canApply =
    (bulkMaterial.trim() || bulkColor.trim() || bulkGroup.trim()) && selectedIds.size > 0;
      ...
      {selectedIds.size > 0 && (
        ...
        >
          ...
          <select
            value={bulkMaterial}
            onChange={(e) => {
              setBulkMaterial(e.target.value);
              setBulkColor("");
            }}
            style={bulkInputSx}
          >
...
function GroupSection({
  ...
  const allVisibleSelected =
    visiblePrinters.length > 0 && visiblePrinters.every((p) => selectedIds.has(p.id));
        ...
        <div style={{ padding: "8px 10px", display: "flex", flexDirection: "column", gap: 4 }}>
          ...
          {group.matched.map((printer) => (
            <div
              ...
              onMouseLeave={(e) => {
                e.currentTarget.style.borderColor = selectedIds.has(printer.id)
                  ? "#1e3a5f"
                  : "#1e2433";
              }}
            >
