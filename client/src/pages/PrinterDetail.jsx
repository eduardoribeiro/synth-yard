...
export default function PrinterDetail() {
  ...
  const fetchData = useCallback(async () => {
    const [printerRes, eventsRes, statsRes, modelsRes, typesRes, colorsRes, groupsRes] =
      await Promise.all([
  ...
  async function submitEditDetails(e) {
    ...
    if (!ip) {
      setDetailsError("IP address is required");
      return;
    }
  ...
  return (
    <div style={{ maxWidth: 720 }}>
      ...
      >
        ...
        >
          {editingName ? (
            <form
              ...
              style={{ display: "flex", gap: 8, alignItems: "center", flex: "1 1 auto" }}
            >
              <input
                autoFocus
                value={nameDraft}
            ...
            <>
              <span style={{ fontWeight: 800, fontSize: 20, color: "#e2e8f0" }}>
                {printer.name}
              </span>
              <button
                onClick={startRename}
        ...
        {nameError && (
          <div style={{ fontSize: 12, color: "#fca5a5", marginBottom: 8 }}>{nameError}</div>
        )}
        ...
        {editingDetails ? (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px 16px" }}>
              ...
              <label style={detailLabelStyle}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setDetailsDraft((d) => ({ ...d, serial_number: e.target.value }))
                  }
                  disabled={savingDetails}
                  placeholder="optional"
                  style={detailInputStyle}
              ...
              <label style={detailLabelStyle}>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setDetailsDraft((d) => ({
                      ...d,
                      loaded_material: e.target.value,
                      loaded_color: "",
                    }))
                  }
                  disabled={savingDetails}
              ...
              <label style={detailLabelStyle}>
                ...
                <select
                  ...
                  style={{
                    ...detailInputStyle,
                    cursor: detailsDraft.loaded_material ? "pointer" : "not-allowed",
                  }}
                >
                  <option value="">— none —</option>
                  {filamentColors
          ...
          >
            <span>
              Model:{" "}
              <span style={{ color: "#94a3b8", fontFamily: "monospace" }}>{printer.model}</span>
            </span>
            <span>
              IP: <span style={{ color: "#94a3b8", fontFamily: "monospace" }}>{printer.ip}</span>
            </span>
            {printer.group_name && (
              <span>
                Group: <span style={{ color: "#94a3b8" }}>{printer.group_name}</span>
              </span>
            )}
            {printer.type && printer.type !== "prusa" && (
              <span>
                Connector: <span style={{ color: "#94a3b8" }}>{printer.type}</span>
              </span>
            )}
      ...
      {stats && (
        ...
        >
          {[
            ...
            >
              <div style={{ fontSize: 22, fontWeight: 800, color: "#e2e8f0", lineHeight: 1.2 }}>
                {value}
              </div>
              <div
                style={{
      ...
      {events.length === 0 && (
        <p style={{ color: "#475569", fontSize: 14 }}>
          No history yet — events are recorded automatically as this printer receives jobs, finishes
          prints, or changes status.
        </p>
      )}
        ...
        <div style={{ marginTop: 32 }}>
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead>
                <tr
                  style={{ color: "#475569", textAlign: "left", borderBottom: "1px solid #1e2433" }}
                >
                  {["Part", "Project", "File", "Started", "Duration", "Parts", "Status"].map(
                    (h) => (
                      <th
                        key={h}
                        style={{ padding: "6px 10px", fontWeight: 600, whiteSpace: "nowrap" }}
                      >
                        {h}
                      </th>
                    ),
            ...
            >
              ...
              <button
                ...
                style={{
                  ...
                  fontSize: 13,
                  fontWeight: 600,
                  cursor: jobPage === jobHistory.total_pages ? "not-allowed" : "pointer",
                }}
              >
                Next →
              </button>
