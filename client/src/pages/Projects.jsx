...
const STATUS_MENU = {
  ...
  active: [
    { label: "Pause project", action: "paused" },
    { label: "Mark complete", action: "complete", danger: true },
  ],
  paused: [
    { label: "Resume project", action: "active" },
    { label: "Mark complete", action: "complete", danger: true },
  ],
...
const PART_STATUS = {
  open: {
    ...
    text: "#60a5fa",
    label: "Open",
    help: "Still needs parts — the scheduler will keep dispatching jobs for it",
  },
  closed: {
    bg: "#14532d",
    text: "#86efac",
    label: "Complete",
    help: "Target quantity reached — no more jobs will dispatch for this part",
  },
};
...
function GcodeUploadPanel({
  ...
  async function handleUpload() {
    ...
    if (bambuNeedsThreemf) {
      setError("Bambu printers require a .3mf file.");
      return;
    }
    if (!partsPerPlate) {
      setError("Enter parts per plate.");
      return;
    }
    if (!model) {
      setError("Select a printer model.");
      return;
    }
    if (amsSlots.length > 0 && amsSlot === "") {
      setError("Select an AMS slot or External Spool.");
      return;
    }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        ...
        {amsSlots.length > 0 && (
          <div>
            ...
            >
              ...
              {amsSlots.map((s) =>
                s.slot === -1 ? (
                  <option key="ext" value="-1">
                    External Spool{s.type ? ` — ${s.type}` : ""}
                  </option>
                ) : (
                  <option key={s.slot} value={String(s.slot)}>
                    Slot {s.slot} — {s.type || "unknown"}
                  </option>
                ),
              )}
            </select>
      ...
      <p style={{ margin: 0, fontSize: 11, color: "#475569" }}>
        Tip: filenames with a model, print time, and weight (e.g.{" "}
        <span className="mono">bracket_MK4S_2h30m_45g.gcode</span>) auto-fill these fields — you can
        adjust them after upload.
      </p>
      ...
      {bambuNeedsThreemf && (
        ...
        >
          Bambu printers require a <strong>.3mf</strong> file exported from Bambu Studio or Orca
          Slicer — plain .gcode files are not supported.
        </p>
      )}
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
        ...
        {filamentTypes.length > 0 ? (
          ...
          >
            <option value="">
              {projectMaterial ? `— project: ${projectMaterial} —` : "— any material —"}
            </option>
            {filamentTypes.map((t) => (
        ...
        {(() => {
          ...
          return (
            ...
            >
              <option value="">
                {projectColor ? `— project: ${projectColor} —` : "— any color —"}
              </option>
              {colorOptions.map((c) => (
            ...
            {groups.map((g) => (
              <label
                ...
                style={{
                  ...
                  cursor: "pointer",
                  fontSize: 12,
                  color: selectedGroups.includes(g) ? "#7dd3fc" : "#64748b",
                }}
              >
                <input
                  type="checkbox"
            ...
            {selectedGroups.length === 0 && (
              <span style={{ fontSize: 11, color: "#334155", fontStyle: "italic" }}>
                {projectGroups?.length > 0
                  ? `(inherits project: ${projectGroups.join(", ")})`
                  : "all groups"}
              </span>
            )}
          </div>
...
function GcodeEstimateRow({
  ...
  const [selectedGroups, setSelectedGroups] = useState(() => {
    try {
      return gc.allowed_groups ? JSON.parse(gc.allowed_groups) : [];
    } catch (_) {
      return [];
    }
  });
  ...
  useEffect(() => {
    ...
    try {
      setSelectedGroups(gc.allowed_groups ? JSON.parse(gc.allowed_groups) : []);
    } catch (_) {
      setSelectedGroups([]);
    }
  }, [
  ...
  async function parseFromFilename() {
    ...
    try {
      ...
      if (data.material_grams != null)
        setMaterialDraft(formatMaterialForInput(data.material_grams));
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
        <span style={{ color: "#e2e8f0", fontSize: 12, fontWeight: 600, flexShrink: 0 }}>
          {gc.parts_per_plate}x
        </span>
...
function PartDetailsPanel({
  ...
  async function runDispatchCheck() {
    ...
    try {
      ...
      const data = await res.json();
      setDispatchCheck(
        res.ok ? data : { dispatchable: false, reasons: [data.error || "Check failed"] },
      );
    } catch (err) {
  ...
  async function saveQtys() {
    ...
    if (isNaN(newHave) || newHave < 0) {
      setQtyError("Have must be 0 or more.");
      return;
    }
    if (isNaN(newNeed) || newNeed < 1) {
      setQtyError("Need must be at least 1.");
      return;
    }
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        {qtyError && (
          <p style={{ color: "#f87171", fontSize: 12, margin: "6px 0 0" }}>{qtyError}</p>
        )}
      </div>
...
export default function Projects() {
  ...
  const [showDraft, setShowDraft] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem(SHOW_DRAFT_KEY) || "false");
    } catch (_) {
      return false;
    }
  });
  const [showPaused, setShowPaused] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem(SHOW_PAUSED_KEY) || "false");
    } catch (_) {
      return false;
    }
  });
  const [showCompleted, setShowCompleted] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem(SHOW_COMPLETED_KEY) || "false");
    } catch (_) {
      return false;
    }
  });
  ...
  useEffect(() => {
    ...
    fetch("/api/filaments/colors")
      .then((r) => r.json())
      .then(setFilamentColors)
      .catch(() => {});
    fetch("/api/groups")
      .then((r) => r.json())
      .then((groups) => setAllGroups(groups.map((g) => g.name)))
      .catch(() => {});
  }, []);

  function toggleShowDraft(v) {
    setShowDraft(v);
    localStorage.setItem(SHOW_DRAFT_KEY, JSON.stringify(v));
  }
  function toggleShowPaused(v) {
    setShowPaused(v);
    localStorage.setItem(SHOW_PAUSED_KEY, JSON.stringify(v));
  }
  function toggleShowCompleted(v) {
    setShowCompleted(v);
    localStorage.setItem(SHOW_COMPLETED_KEY, JSON.stringify(v));
  }
  ...
  function dropProject(targetId) {
    ...
    if (!projectDragSrc || projectDragSrc === targetId) {
  ...
  function dropPart(targetId) {
    ...
    if (!partDragSrc || partDragSrc === targetId) {
      setPartDragSrc(null);
      return;
    }
      ...
      <div>
        ...
        {dupModal &&
          createPortal(
            <div
              ...
              onClick={() => {
                if (!duplicating) setDupModal(null);
              }}
            >
              <div
                style={{
              ...
              >
                ...
                <div style={{ color: "#64748b", fontSize: 13, marginBottom: 14 }}>
                  All parts and G-code files will be copied. The new project starts as a draft with
                  all quantities reset to zero.
                </div>
                <label
                  style={{ color: "#94a3b8", fontSize: 12, display: "block", marginBottom: 6 }}
                >
                  New project name
                </label>
                <input
                  ...
                  onKeyDown={(e) => {
                    ...
                    if (e.key === "Escape" && !duplicating) setDupModal(null);
                  }}
            ...
            {draftCount > 0 && (
              ...
              >
                <input
                  type="checkbox"
                  checked={showDraft}
                  onChange={(e) => toggleShowDraft(e.target.checked)}
                  style={{ accentColor: "#3b82f6" }}
                />
            ...
            {pausedCount > 0 && (
              ...
              >
                <input
                  type="checkbox"
                  checked={showPaused}
                  onChange={(e) => toggleShowPaused(e.target.checked)}
                  style={{ accentColor: "#3b82f6" }}
                />
        ...
        {!loading && projects.length === 0 && (
          <EmptyState
            ...
            hint={
              <>
                Projects are how work flows through the farm:&nbsp; a{" "}
                <strong style={{ color: "#cbd5e1" }}>Project</strong> contains{" "}
                <strong style={{ color: "#cbd5e1" }}>Parts</strong> (what to print and how many),
                each part gets <strong style={{ color: "#cbd5e1" }}>G-code</strong> uploaded per
                printer model, and the scheduler dispatches{" "}
                <strong style={{ color: "#cbd5e1" }}>Jobs</strong> to idle printers until every part
                hits its target quantity. Start with “+ New Project” above.
              </>
            }
          />
        )}
          ...
          {visibleProjects.map((p) => {
            ...
            return (
              <div
                ...
                onDragOver={(e) => {
                  e.preventDefault();
                  if (!isDragging) setProjectDragOver(p.id);
                }}
                onDrop={(e) => {
                  e.preventDefault();
                  dropProject(p.id);
  ...
  let projectGroups = [];
  try {
    projectGroups = detailProject.allowed_groups ? JSON.parse(detailProject.allowed_groups) : [];
  } catch (_) {}
    ...
    <div>
        ...
        {editingProjectName ? (
          ...
          <>
            ...
            <button
              onClick={() => {
                renameEscapedRef.current = false;
                setProjectNameDraft(detailProject.name);
      ...
      {allGroups.length > 0 && (
        ...
        >
          ...
          {allGroups.map((g) => (
            <label
              ...
              style={{
                ...
                cursor: "pointer",
                fontSize: 12,
                color: projectGroups.includes(g) ? "#7dd3fc" : "#64748b",
              }}
            >
              <input
                type="checkbox"
          ...
          {projectGroups.length === 0 && (
            <span style={{ fontSize: 11, color: "#334155", fontStyle: "italic" }}>all groups</span>
          )}
          {projectGroups.length > 0 && (
            <span style={{ fontSize: 11, color: "#475569", fontStyle: "italic" }}>
              applies to all gcodes in this project unless overridden per-gcode
      ...
      {parts.map((part) => {
        ...
        const pct =
          part.target_qty > 0 ? Math.round((part.completed_qty / part.target_qty) * 100) : 0;
        ...
        return (
          <div
            ...
            onDragOver={(e) => {
              e.preventDefault();
              if (!isPartDragging) setPartDragOver(part.id);
            }}
            onDrop={(e) => {
              e.preventDefault();
              dropPart(part.id);
          ...
          >
            <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
              {/* Name + drag handle */}
              <div
                style={{ display: "flex", alignItems: "center", gap: 6, width: 200, flexShrink: 0 }}
              >
                <span
                  title="Drag to reorder"
                  aria-hidden="true"
